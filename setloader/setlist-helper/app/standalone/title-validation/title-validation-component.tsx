...
export function TitleValidationComponent({ onComplete, onError }: TitleValidationComponentProps) {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        console.log('Loading user data for standalone component...')
        
        // Load catalog from standalone endpoint
        const catalogResponse = await fetch(`${API_BASE_URL}/standalone/user-catalog`, {
          headers: {
            'X-Secret': API_SECRET
          }
        })
        ...
        console.log('User data loaded for standalone')
      } catch (error) {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/standalone/title-validation`, {
        method: 'POST',
        body: formData
      })
      ...
      if (result.success) {
        ...
        // Don't call onComplete yet - let user map titles first
      } else {
  ...
  const handleQuickMapping = async (title: string, mappedTitle: string) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/standalone/save-mapping`, {
        method: 'POST',
        headers: {
          'X-Secret': API_SECRET,
