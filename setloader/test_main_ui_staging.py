...
"""
Test the main UI staging system to verify it properly handles mapped songs.
"""
...
def test_main_ui_staging_workflow():
    """Test the main UI staging workflow."""
    print("\n=== Testing Main UI Staging Workflow ===")
    ...
    # Step 2: Extract titles and create validation data (simulating main UI)
    print("\n2. Creating validation data (main UI style)...")
    ...
    # Create validation data in the format the main UI uses
    validation_data = {
        "sets": [{
            "name": "Extracted Set",
    ...
    # Step 5: Re-run validation to get updated results (simulating main UI re-validation)
    ...
    # Step 6: Check if the mapped song has the correct song_id in the validation result
    print("\n6. Checking mapped song in validation result...")
    ...
    print(f"  - validated: {party_song.get('validated')}")
    print(f"  - validated_title: {party_song.get('validated_title')}")
    print(f"  - song_id: {party_song.get('song_id')}")
    print(f"  - status: {party_song.get('status')}")
    
    if party_song.get('song_id') != 178:
        print(f"❌ FAILURE: song_id is {party_song.get('song_id')}, expected 178")
        return False
    
    print("✅ SUCCESS: song_id is correctly set to 178")
    
    # Step 7: Run song extraction with the updated validation data
    print("\n7. Running song extraction with updated validation data...")
...
def main():
    """Run the test."""
    print("=== Testing Main UI Staging System ===")
    ...
    try:
        ...
        # Step 2: Test the staging workflow
        success = test_main_ui_staging_workflow()
        
        if success:
            print("\n🎉 MAIN UI STAGING TEST PASSED!")
        else:
            print("\n❌ MAIN UI STAGING TEST FAILED!")
